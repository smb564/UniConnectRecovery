...
public class UserResource {
    ...
    /**
     * POST  /users/{id}/notification : Add notifications to student
     *
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/users/{login}/notification")
    @Timed
    public ResponseEntity<UserNotification> addNotifcation(@Valid @RequestBody UserNotification userNotification, @PathVariable String login) throws URISyntaxException {
        log.debug("REST request to add notification to User : {} , {}", userNotification, login);

        UserNotification result = userService.addNotification(userNotification, login);
